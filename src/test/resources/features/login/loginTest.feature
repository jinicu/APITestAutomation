Feature: Application Login Tests

  @login @TOKEN
  Scenario Outline: Successful Login <TCID>
    Given request url with endpoint: <endpoint>
    And request payload: <payload>
    And user email and password of user: <userID>
    When user send request with method: <requestMethod>
    Then response status code result should be: <responseCode>
    And response body contains: <responseBody>

    Examples:
      | endpoint   | payload    | userID     | requestMethod | responseCode | responseBody   | TCID       |
      | login      | login      | us00001    | POST          | 200          | token~notEmpty | TC00001    |

  @login @TOKEN
  Scenario Outline: Unsuccessful Login: <Description> <TCID>
    Given request url with endpoint: <endpoint>
    And request payload: <payload>
    And user with email for: <emailUID> and password for: <passwordUID>
    When user send request with method: <requestMethod>
    Then response status code result should be: <responseCode>
    And response body contains: <responseBody>

    Examples:
      | endpoint   | payload    | emailUID    | passwordUID | requestMethod | responseCode | responseBody                       | Description                |TCID       |
      | login      | login      |             |             | POST          | 400          | error~Missing email or username    | Missing Email and Password |TC00002    |
      | login      | login      |             | us00001     | POST          | 400          | error~Missing email or username    | Missing Email              |TC00003    |
      | login      | login      | us00001     |             | POST          | 400          | error~Missing password             | Missing Password           |TC00004    |
      | login      | login      | invalidUser | us00001     | POST          | 400          | error~user not found               | Invalid email              |TC00005    |
      | login      | login      | us00001     | invalidUser | POST          | 400          | error~Invalid username or password | Invalid Password           |TC00006    |